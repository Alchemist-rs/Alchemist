...
pub fn package(pkgincoming : Result<JSON<PkgIncoming>, SerdeError>) -> Result<JSON<Package>, String> {
    match pkgincoming {
        Err(why) => {
            Err(json!({
                        "error": format!("There was an error parsing your JSON request: \"{:?}\"", why),
                        "note": "You probably messed up the syntax, check the docs @ https://alchemist.rs\
                            to ensure your sending a valid JSON request."
                      })
                        .to_string())
        },
        Ok(pkg_incoming) => {
